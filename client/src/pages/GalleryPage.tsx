...
export default function GalleryPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      galleryService.getAll({ country: countryFilter || undefined, type: typeFilter || undefined, limit: 100 }).then((r) => r.data),
  });
    ...
    <main className={styles.page}>
      <div className={styles.container}>
        <header className={styles.header}>
          ...
          <p className={styles.subtitle}>Photographs and moments captured along the way</p>
        </header>
        ...
        {isLoading ? (
          <div className={styles.masonry}>
            {Array.from({ length: 12 }).map((_, i) => (
              <div key={i} className={`skeleton ${styles.skeletonItem}`} style={{ height: `${200 + (i % 3) * 80}px` }} />
            ))}
          </div>
        ) : (
          <div className={styles.masonry}>
